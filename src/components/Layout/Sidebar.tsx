...
import {
  LayoutDashboard, FlaskConical, Target, ShieldCheck,
  Bell, LogOut, Activity, Sun, Moon, Globe,
} from 'lucide-react'
  ...
  return (
    ...
    >
      ...
      >
        ...
        <div>
          <p className="font-mono text-sm font-bold leading-tight text-text-bright">IDS Lab</p>
          <p className="font-mono text-[8px] uppercase tracking-widest text-text-dim">Assessment</p>
        </div>
      </div>
      ...
      <nav className="flex-1 space-y-0.5 px-2 py-4">
        {nav.map(({ to, icon: Icon, label }) => (
          <NavLink
            ...
            className={({ isActive }) =>
              clsx(
                'group flex items-center gap-3 rounded-lg px-3 py-2.5 text-xs font-mono transition-all border',
                isActive
      ...
      <div className="px-3 pb-4 space-y-3" style={{ borderTop: '1px solid var(--border)', paddingTop: '12px' }}>
        ...
        <div>
          <div className="flex items-center gap-1.5 px-1 mb-1.5">
            ...
            <span className="font-mono text-[9px] uppercase tracking-widest text-text-dim">Language</span>
          </div>
        ...
        <button
          onClick={toggleTheme}
          className="flex w-full items-center gap-2 rounded-lg px-3 py-2 text-xs font-mono transition-all"
          style={{
            background: 'var(--bg-3)',
        ...
        >
          {theme === 'dark'
            ? <><Sun size={12} style={{ color: 'var(--warn)' }} /><span>Light mode</span></>
            : <><Moon size={12} style={{ color: 'var(--accent)' }} /><span>Dark mode</span></>
          }
        </button>
        ...
        <button
          ...
          className="flex w-full items-center gap-2 rounded-lg px-3 py-2 text-xs font-mono transition-all hover:bg-danger\/10 hover:text-danger"
